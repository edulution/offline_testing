...
CREATE OR REPLACE PROCEDURE public.spassignmembership (i_module character varying)
LANGUAGE 'plpgsql'
AS $BODY$
DECLARE
    sPos varchar(10);
...
BEGIN
    ...
    SELECT
        id AS user_id
    FROM
        ext.kolibriauth_facilityuser
    WHERE
        id NOT IN (
            SELECT
                user_id
            FROM
                ext.kolibriauth_role
            WHERE
                kind IN ('coach', 'admin'))
            LOOP
                sPos := '20';
                ...
                ELSE
                    ...
                    SELECT
                        course_family
                    FROM
                        get_next_course (currSortOrder, i_module) INTO nextCourseFamily;
                    sPos := '60';
                END IF;
                sPos := '70';
                ...
                SELECT
                    *
                FROM
                    get_user_classrooms (learnerRow.user_id)
                    LOOP
