...
def get_highest_passed_test(isession, iuser_id, imodule):
    ...
    # Fetch the top row
    tests_passed_query = (
        isession.query(TestResult)
        .filter_by(user_id=iuser_id, passed=True, module=imodule)
...
def get_next_course(isession, itestresult, imodule="numeracy"):
    ...
    if itestresult is None:
        lowest_course_query = (
            isession.query(Course)
            .filter_by(module=imodule)
    ...
    if not itestresult.passed:
        ...
        if next_course_query.first() is None:
            return current_course_query.one()
