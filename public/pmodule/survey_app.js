...
angular.module('passProtect', ['ngAnimate', 'ngSanitize', 'ui.bootstrap', 'ui', 'ui.filters', 'angular-md5'])
    ...
    .controller('MainCtrl', ($scope, $timeout, $http, $uibModal, $location, $log, $document, md5, passwordService) => {
        ...
        $scope.init = () => {
            ...
            /*Empty object for surveyResponse*/
        ...
        /*this can be overidden with an any array of properties that the surveyResponse object contains*/
        ...
        const check_response_already_exists = (surveyResponse) => {
            ...
            let surveyResponse_concat = concat_props(surveyResponse)

            /*check if the response submitted exists in list of existing responses*/
            if ($scope.existingResponses.indexOf(surveyResponse_concat) >= 0) {
        ...
        $scope.check_reponse_before_submit = () => {
            ...
            let currentUser = $scope.users.find(user => { return user.username == $scope.surveyResponse.username })

            /*add the user_id as an attribute to the surveyResponse object*/
            $scope.surveyResponse.user_id = currentUser.user_id

            /*set the test date of the response to the server date*/
            $scope.surveyResponse.test_date = $scope.serverDate

            /*check if the response already exists*/
            check_response_already_exists($scope.surveyResponse)
        }
        ...
        $scope.display_username = () => {
            if ($scope.surveyResponse.username) {
                let currentUser = $scope.users.find(user => { return user.username == $scope.surveyResponse.username })
        ...
        $scope.submit = () => {
            $http.post("/api/submit_test", $scope.surveyResponse).then(success =>
        ...
        $scope.overwrite_test = () => {
            $http.post("/api/overwrite_test", $scope.surveyResponse).then(res => {
                /*redirect to sucessful submission page*/
                if (res.status == 200) {
        ...
        $timeout(() => {
            ...
            $scope.$watch('form.testForm.username.$error.whitelist', () => {
                ...
                if (!$scope.form.testForm.username.$error.whitelist) {
                    ...
                    let currentUser = $scope.users.find(user => { return user.username == $scope.surveyResponse.username })
                    /*var currentTest = { $scope.surveyResponse.test, $scope.surveyResponse.course, $scope.surveyResponse.course }*/
                    /*Get the test, course and module of the surveyResponse object as an object*/
                    let currentTest = (({ test, course, module }) => ({ test, course, module }))($scope.surveyResponse)
